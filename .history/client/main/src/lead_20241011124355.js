...
const Lead = () => {
  ...
  const handleCancelEvent = async (eventid) => {
    const modifiedEvents = events.map((event, index) => ({
      ...event,
      eventid: index + 1 // Temporary unique ID for demonstration
    }));
    // Sample modification of the events array
  

    if (!eventid) {
      console.error("No event ID provided");
      return; // Early exit if event ID is undefined
    }
  ...
  {events.map(event => (
    <div key={event.eventid}>
      ...
      <button onClick={() => handleCancelEvent(event.eventid)}>Cancel</button>
    </div>
  ))}
            ...
            {events.length > 0 ? (
              events.map((event, index) => (
                <div key={index} className="mb-4 flex justify-between items-center">
                  <p>{event.eventname}</p>
                  <button
                    className="px-4 py-2 bg-red-600 hover:bg-red-700 text-white font-bold rounded-lg shadow-lg transition duration-200 ease-in-out"
              ...
              <p>No events available for cancellation.</p>
            )}
            <button
              className="mt-4 px-4 py-2 bg-gray-600 hover:bg-gray-700 text-white font-bold rounded-lg shadow-lg transition duration-200 ease-in-out"
