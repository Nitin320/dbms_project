...
const Lead = () => {
  ...
  const handleCancelEvent = async (eventId) => {
    if (confirm("Are you sure you want to cancel this event?")) {
      try {
        const response = await fetch(`http://127.0.0.1:5000/api/deleteEvent/${eventId}`, {
          method: 'DELETE',
          headers: {
            'Content-Type': 'application/json',
          },
        ...
        if (result.message === 'Event deleted successfully') {
          ...
          setEvents(events.filter(event => event.id !== eventId)); // Update the events state
        } else {
        ...
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50">
          <div className="bg-gray-800 text-white p-6 rounded-lg shadow-lg w-11/12 max-w-lg max-h-[90vh] overflow-y-auto hide-scrollbar">
            ...
            <div className="flex justify-end mt-6">
              <button
                className="px-6 py-2 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-lg shadow-lg transition duration-200 ease-in-out"
