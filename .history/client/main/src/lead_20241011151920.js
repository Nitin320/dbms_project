...
const Lead = () => {
  ...
  const deleteEvent = async (eventId) => {
    // Log the event ID to check if it's being passed correctly
    console.log("Deleting event with ID:", eventId); 
    
    if (!eventId) {
      alert('Event ID is required');
      return; // Prevent further execution if ID is not available
    }
    ...
    try {
      const response = await fetch('http://127.0.0.1:5000/api/deleteEvent', {
        ...
        body: JSON.stringify({ eventId }) // Ensure you're sending the ID correctly
      });
      ...
      if (response.ok) {
        ...
        console.error('Error:', errorData.message); // Log error message
        alert(`Error: ${errorData.message}`); // Provide user feedback
      }
    } catch (error) {
      console.error('Fetch error:', error);
      alert('An error occurred while deleting the event. Please try again.'); // Provide user feedback
    }
  };
          ...
          <div className="bg-gray-800 text-white p-6 rounded-lg shadow-lg w-11/12 max-w-lg max-h-[90vh] overflow-y-auto hide-scrollbar">
            ...
            events.map((event) => (
              <div key={event.id} className="flex justify-between items-center p-4 bg-gray-700 rounded-lg">
                ...
                <button 
                  className="py-1 px-3 bg-red-500 rounded-lg hover:bg-red-600 transition duration-200" 
                  onClick={() => deleteEvent(event.id)} // Pass the event ID directly
                >
                  Cancel
                </button>
              </div>
