...
def register_routes(app, db):
    ...
    def delete_event(event_id):
        ...
        event = events.query.filter_by(eventid=event_id, completed=False).first()

        if event:
            # If event is not completed, delete it
            db.session.delete(event)
            db.session.commit()
            return jsonify({'message': 'Event deleted successfully'}), 200
        else:
            return jsonify({'message': 'Event not found or already completed'}), 404
